...
icon = pygame.image.load('32px Icon.png')
...
def game_over_text(score_value):
    ...
    if GO_has_run == False:
        ...
        if highscore == score_value:
            HS_message = " You reached the highscore!"
        highscore_file.close()

        if highscore > score_value:
            HS_message = (" The highscore is " + str(highscore))

        if highscore < score_value:
            HS_message = " You set a new highscore!"
            highscore_file = open("Highscore.txt", "w")
            highscore_file.write(str(score_value))
            highscore_file.close()

    over_text = small_font.render(HS_message, True, (255, 255, 255))
